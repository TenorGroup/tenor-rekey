...
# colorama and prompt_toolkit are used only by the interactive CLI (colored
# output + the completer classes at the bottom of this file), never by
# ChameleonCom / ChameleonCMD. They are imported optionally so the vendored
# package loads on a bare interpreter (the daemon needs pyserial only).
try:
    ...
    Completer = NestedCompleter = WordCompleter = object
    Completion = Document = None

from .chameleon_enum import Status

# Colorama shorthands (inert empty strings when colorama is not installed)
try:
    import colorama
