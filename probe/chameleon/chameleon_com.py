...
class ChameleonCom:
    ...
    def send_cmd_sync(self, cmd: int, data: Union[bytes, None] = None, status: int = 0,
        ...
        # wait cmd start process (bounded + close-safe: a transport thread may
        # clear wait_response_map via close() during either wait window, which
        # would otherwise spin here forever or KeyError in the loop below)
        start_deadline = time.time() + timeout + 1
        while cmd not in self.wait_response_map:
            if time.time() > start_deadline:
                raise TimeoutError(f"CMD {cmd} start timeout")
            time.sleep(0.01)
        # wait response data set
        while True:
            entry = self.wait_response_map.get(cmd)
            if entry is None:
                # close() cleared the map mid-wait: the port is gone
                raise TimeoutError(f"CMD {cmd} lost (connection closed)")
