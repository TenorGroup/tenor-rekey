...
final class L10n {
    ...
    static let table: [String: [String: String]] = [
        ...
        "format_q":      ["vi": "đưa thẻ này về mặc định?", "en": "format this card to factory?", "zh": "将此卡格式化为出厂状态？", "ja": "このカードを工場出荷状態に初期化しますか？"],
        "format_msg":    ["vi": "xóa toàn bộ dữ liệu và đưa các khóa về ff. không lấy lại được. block 0 (uid) giữ nguyên.", "en": "erases all data and resets keys to ff. cannot be undone. block 0 (uid) is left intact.", "zh": "清除所有数据并将密钥重置为 ff，不可撤销。第 0 块（uid）保持不变。", "ja": "全データを消去し鍵を ff に戻します。元に戻せません。ブロック0（uid）はそのまま。"],
        "clone_q":       ["vi": "ghi đè lên thẻ trên đầu đọc?", "en": "overwrite the card on the reader?", "zh": "覆盖读卡器上的卡片？", "ja": "リーダー上のカードを上書きしますか？"],
        "clone_msg":     ["vi": "ghi dữ liệu lên thẻ đích. không lấy lại được. nếu bật ghi khóa/uid, access-bits sai có thể làm hỏng hẳn một sector.", "en": "writes the source data onto the target card and cannot be undone. with keys/uid enabled, bad access bits can permanently lock a sector.", "zh": "将源数据写入目标卡，不可撤销。启用密钥/uid 写入时，错误的访问位可能永久锁定扇区。", "ja": "ソースのデータを対象カードに書き込みます。元に戻せません。鍵/uid 書き込みが有効な場合、不正なアクセスビットでセクタが永久にロックされることがあります。"],
        ...
        "no_keys_msg":   ["vi": "không có khóa nào trong từ điển mở được thẻ này. nếu có khóa, hãy thêm vào cài đặt rồi giải mã lại. nếu không, bản này chưa đọc được thẻ vì chưa hỗ trợ dò khóa lạ.", "en": "no key in the dictionary matched this card. if you have a key, add it in settings (key dictionary) and decode again. without a key, this build cannot read the card yet - key recovery for a fully unknown card is not available.", "zh": "字典中没有匹配此卡的密钥。如果您有密钥，请在设置（密钥字典）中添加后重新解码。若没有密钥，此版本暂时无法读取该卡 - 尚不支持对完全未知卡的密钥恢复。", "ja": "辞書内にこのカードと一致する鍵がありませんでした。鍵をお持ちなら設定（鍵辞書）で追加して再度デコードしてください。鍵がない場合、このビルドではまだ読み取れません - 全く未知のカードの鍵復元は未対応です。"],
        ...
        "decode_to_read":["vi": "thẻ trên đầu đọc không phải thẻ nguồn. bấm giải mã để đọc thẻ này, hoặc bấm ghi để chép thẻ nguồn sang.", "en": "the card on the reader differs from the document - decode to read it, or write to copy the document onto it", "zh": "读卡器上的卡与文档不同 - 解码以读取它，或写入以将文档复制到它上面", "ja": "リーダー上のカードは文書と異なります - デコードで読み取るか、書き込みで文書をコピー"],
        ...
        "write_trailers_hint": ["vi": "chép khóa của thẻ nguồn sang thẻ đích. tắt mục này để giữ khóa của thẻ đích và chỉ ghi dữ liệu.", "en": "copy the source keys onto the target. off = keep the target's own keys, write data only.", "zh": "把源卡的密钥写入目标卡。关闭 = 保留目标卡原有密钥，仅写数据。", "ja": "ソースの鍵を対象カードに書き込む。オフ = 対象カードの鍵を保持しデータのみ書き込む。"],
        "write_uid_hint": ["vi": "đổi cả uid của thẻ. hầu hết thẻ không cho ghi uid, chỉ thẻ magic mới ghi được.", "en": "also overwrite the card uid. most cards refuse this; magic (uid-writable) cards only.", "zh": "同时覆盖卡片 uid。多数卡片不允许，仅限魔术卡（uid 可写）。", "ja": "カードの uid も上書き。多くのカードは拒否、magic（uid 書込可）カードのみ。"],
        ...
        "target_real_hint": ["vi": "ghi dữ liệu lên thẻ thật bằng khóa đã biết. block 0 giữ nguyên.", "en": "write the document onto a real card with its known keys; block 0 is left intact.", "zh": "用已知密钥把文档写入真实卡，第 0 块保持不变。", "ja": "既知の鍵で文書を実カードに書き込み、ブロック0はそのまま。"],
        ...
        "assumed_warning": ["vi": "một số khóa chỉ được suy ra từ khóa còn lại, không phải đọc trực tiếp từ thẻ. nếu đầu đọc thật dùng các khóa đó để xác thực, thẻ sao có thể bị từ chối.", "en": "some keys are assumed (copied from the other slot), not read. if the real reader authenticates with that key, the clone may be rejected.", "zh": "部分密钥是推测得出的（从另一槽复制），并非读取所得。若真实读卡器用该密钥验证，克隆卡可能被拒。", "ja": "一部の鍵は読み取りではなく推定（もう一方から複製）です。実機がその鍵で認証する場合、複製は拒否される可能性があります。"],
        ...
        "recover_confirm_title": ["vi": "kiểm tra lại model", "en": "confirm the device model", "zh": "确认设备型号", "ja": "デバイスの型番を確認"],
        "recover_confirm_msg":   ["vi": "nạp nhầm model có thể làm hỏng thiết bị. chỉ tiếp tục nếu đây đúng là model đã chọn:", "en": "flashing the wrong model can brick the device. continue only if this is the model you selected:", "zh": "刷错型号可能导致设备变砖。仅当这是你选择的型号时才继续：", "ja": "型番を誤ると文鎮化する恐れがあります。選択した型番である場合のみ続行してください:"],
        "offline_note":      ["vi": "không lấy được bản mới nhất:", "en": "could not fetch the latest release:", "zh": "无法获取最新版本：", "ja": "最新リリースを取得できません:"],
        "firmware_warning":  ["vi": "không rút thiết bị hoặc tắt ứng dụng khi đang cập nhật. thiết bị sẽ tự kiểm tra chữ ký firmware. công cụ chỉ nạp gói ứng dụng (app-only) từ bản phát hành chính chủ và kiểm tra hash ảnh. gói full (bootloader) sẽ bị từ chối để tránh làm hỏng máy.", "en": "do not unplug the device or quit the app during the update. the device itself verifies the firmware signature; this tool flashes only application-only official-release packages and checks the image hash. a full (bootloader) package is refused to avoid bricking.", "zh": "更新期间请勿拔出设备或退出应用。设备自身校验固件签名；本工具仅刷写官方发布的纯应用（app-only）包并核对镜像哈希。为避免变砖，完整（引导程序）包会被拒绝。", "ja": "更新中はデバイスを抜いたりアプリを終了しないでください。署名の検証はデバイス自身が行います。本ツールは公式リリースのアプリのみ（app-only）パッケージを書き込み、イメージのハッシュを照合します。文鎮化を防ぐため、フル（ブートローダ）パッケージは拒否されます。"],
        ...
        "firmware_reboot":   ["vi": "máy đang khởi động lại với firmware mới. số phiên bản sẽ cập nhật sau vài giây.", "en": "the device is rebooting into the new firmware. the version updates in a few seconds.", "zh": "设备正在重启进入新固件，版本将在几秒后更新。", "ja": "デバイスは新しいファームウェアで再起動しています。バージョンは数秒後に更新されます。"],
        ...
        "dfu_recover_hint":  ["vi": "thiết bị đang ở bootloader nên không đọc được model. chọn Ultra hoặc Lite để tải firmware và nạp lại.", "en": "the device is in the bootloader, so its model cannot be read. choose Ultra or Lite to download and flash-recover it.", "zh": "设备处于引导程序模式，无法读取其型号。选择 Ultra 或 Lite 以下载并刷写恢复。", "ja": "デバイスはブートローダにあり型番を読めません。Ultra か Lite を選んでダウンロードし復旧書き込みしてください。"],
        "firmware_failed":   ["vi": "nạp firmware thất bại", "en": "firmware update failed", "zh": "固件更新失败", "ja": "ファームウェア更新に失敗しました"],
        "firmware_retry_hint": ["vi": "thiết bị có thể đang ở bootloader. chọn Ultra hoặc Lite để nạp lại firmware.", "en": "the device is likely in the bootloader. choose Ultra or Lite to flash-recover it again.", "zh": "设备可能仍处于引导程序。选择 Ultra 或 Lite 再次刷写恢复。", "ja": "デバイスはブートローダのままの可能性があります。Ultra か Lite を選んで再度復旧書き込みしてください。"],
        ...
        "saved_cards_hint":  ["vi": "giải mã hoặc nhập dữ liệu thẻ, rồi lưu vào đây để dùng lại", "en": "decode or import a card, then save it here to reuse", "zh": "解码或导入一张卡片后保存到这里以便复用", "ja": "カードをデコードまたはインポートし、ここに保存して再利用します"],
